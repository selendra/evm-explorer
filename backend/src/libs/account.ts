...
export const processEvmAccountInfo = async (
  api: Web3,
  client: Client,
  address: string,
  ...
  const sql = `INSERT INTO evm_account (
    ...
    DO UPDATE SET
     ...
     timestamp = EXCLUDED.timestamp,
     block_height= EXCLUDED.block_height
    WHERE EXCLUDED.account_id = evm_account.account_id;`;

  try {
    await client.query(sql, data);
    logger.info(
      loggerOptions,
      `Updated account info for transaction involved address ${address}`,
    );
  } catch (error) {
    logger.error(
      loggerOptions,
      `Error Updated account info #${block_height}: ${error}`,
    );
    const scope = new Sentry.Scope();
    scope.setTag('blockNumber', block_height);
