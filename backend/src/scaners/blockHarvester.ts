...
import { logger, ScanerConfig, wait, substrateProvider, isNodeSynced , getClient, dbQuery } from '../utils';
import { healthCheck, harvestBlocks, harvestBlocksSeq } from '../libs/block';
import { backendConfig } from '../config';

const scanerName = 'blockHarvester';
...
const scaner = async (delayedStart: boolean) => {
  if (delayedStart) {
    logger.info(
      loggerOptions,
      `Delaying block harvester scaner start for ${config.startDelay / 1000}s`,
    );
